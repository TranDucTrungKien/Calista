...
const seedFileContent = fs.readFileSync('d:\\Calista\\backend\\src\\seed.js', 'utf8');

// Define dummy helpers
...
function getExpandedWords(words) {
  const expanded = new Set(words);
  for (const word of words) {
    for (const [key, syns] of Object.entries(SYNONYMS)) {
      if (word === key || removeAccents(word) === removeAccents(key)) {
...
function findBestMatch(productName, originalImage) {
  ...
  if (productName === 'Serum vitamin C sáng da') {
    return { bestFile: 'Serum Calista Vitamin C mờ thâm 50ml.png', maxScore: 999, method: 'override' };
  }
  ...
  if (origName) {
    ...
    if (maxScore > 10) {
      return { bestFile, maxScore, method: 'orig_image' };
    }
  }
  ...
  return { bestFile, maxScore, method: 'product_name' };
}
...
for (const cat of CATEGORIES) {
  for (const p of cat.products) {
    const { bestFile, maxScore, method } = findBestMatch(p.name, p.images);
    mapping.push({
      productName: p.name,
      originalImage: p.images ? p.images[0] : null,
      matchedFile: bestFile,
      score: maxScore,
      method: method
    });
    fileCounts[bestFile] = (fileCounts[bestFile] || 0) + 1;
  }
}

// Print results table
console.log("No. | Product Name | Original Image | Matched File | Score | Method");
console.log("---|---|---|---|---|---");
mapping.forEach((m, idx) => {
  console.log(`${idx + 1} | ${m.productName} | ${m.originalImage} | ${m.matchedFile} | ${m.score.toFixed(1)} | ${m.method}`);
});

console.log("\nDuplicate usage check:");
for (const [file, count] of Object.entries(fileCounts)) {
  if (count > 1) {
    console.log(`- ${file} is used ${count} times!`);
  }
}
